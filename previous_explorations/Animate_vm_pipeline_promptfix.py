...
def _extract_image_urls(result: dict) -> list[str]:
    ...
    def _looks_like_image_url(u: str) -> bool:
        u2 = (u or "").lower()
        return ("fal.media" in u2) or u2.endswith((".png", ".jpg", ".jpeg", ".webp")) or ("/images/" in u2)
...
def render_pipeline_ui():
    ...
    with right:
        ...
        if st.session_state.pipe_step2_url:
            ...
            st.image(st.session_state.pipe_step2_url, width='stretch')

        if st.session_state.pipe_step3_url:
            st.markdown("**Final (Step 3) output**")
            st.image(st.session_state.pipe_step3_url, width='stretch')
